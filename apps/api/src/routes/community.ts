...
const community = new Hono<{ Bindings: Env; Variables: Record<string, any> }>();
...
community.get("/", async (c) => {
  ...
  try {
    ...
    const feed = (results as any[]).map(p => ({
      ...
      description: p.description?.slice(0, 200), subject: p.subject,
      lociCount: p.lociCount, createdAt: p.createdAt,
    }));
...
// GET /api/p/:slug — Public read-only walkthrough (no auth required)
community.get("/p/:slug", async (c) => {
  ...
  try {
    ...
    return c.json({ id: palace.id, name: palace.name, description: palace.description,
      subject: palace.subject, loci: palace.loci, lociCount: palace.lociCount, spatialMap: palace.spatialMap || [] });
  } catch (e: any) {
    return c.json({ error: "internal_error" }, 500);
  }
});

// POST /api/community/share/:id — Toggle public sharing
community.post("/share/:id", authMiddleware, async (c) => {
  ...
  try {
    ...
    await db.update(TABLES.memoryPalaces).set({ isPublic: newPublic, updatedAt: new Date() } as any)
      .where(eq(TABLES.memoryPalaces.id, palaceId));
    return c.json({ isPublic: newPublic, shareUrl: newPublic ? `/p/${palace.slug}` : null });
  } catch (e: any) {
